/*
    ...
    那么这个过程实际上可以拆分成 u 到 lca , lca 到 v 
    那么显然 lca 的 dfn 序不会大于 u 和 v 的 dfn 序
    这也就意味着 从u 到 lca 的过程中我们是从 dfn 序大的到 dfn 序小的 
    而从 lca to v 的过程中我们是从 dfn 序小的到 dfn 序大的
    对应着我们线段树上从左到右和从右到左维护的两个信息 lprofit 和 rprofit
    需要注意的是当 u 和 v 处于同一重链的时候 
    需要去判断谁的 dfn 序大 谁的 dfn 序小 从而判断出应该使用 lprofit 还是 rprofit

    在合并的时候 最大值和最小值显然是可以直接向上合并的
    而 lprofit 和 rprofit 不仅要从 左右儿子中选出较大者 还需要根据公式额外计算
    例如 lprofit = max(Llporfit, Llprofit, R_max - Lmin);
    类似的 , rprofit 以及在合并答案的时候都应采用这个逻辑
*/
